...
CmdNode *deleteCommand(CmdNode *head, int cmdNum) {
  ...
  // initialize two CmdNode pointers
  CmdNode* freeing;
  CmdNode* prev;
  ...
  if(head != NULL){
    ...
    // if the requested node is out of bound, return head
      ...
      // loops until the requested node is gotten
      for(int i = 0; i < cmdNum; i++){
        prev = p;
        p = p -> next;
      }
      // set a pointer to the deletion node and links the previous node to the next one
      prev -> next = p -> next;
      freeing = p;
    }
    // freeing the desired node
    free(freeing);
  }
